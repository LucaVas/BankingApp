...
class MainWindow(tk.Tk):
    def __init__(self, holder, account, bank):
        ...
        # buttons
        self.top_up_button = tk.Button(self, text=self.top_up_button_text, padx=self.btn_padx, pady=self.btn_pady, state="active", command=self.open_top_up_window).grid(row=2,column=0)

        self.transfer_button = tk.Button(self, text=self.transfer_button_text, padx=self.btn_padx, pady=self.btn_pady, state="active", command=self.open_transfer_window).grid(row=3,column=0)
        
        self.log_out_button = tk.Button(self, text=self.log_out_button_text, padx=self.btn_padx, pady=self.btn_pady, state="active", command=self.log_out).grid(row=4,column=2)
