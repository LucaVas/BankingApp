...
class MainWindow(MainGui):
    ...
    def __init__(self, holder: Holder, account: Account, bank: Bank) -> None:
        ...
        # Button: "Top-Up"
        self.button_top_up = ctk.CTkButton(self, width=40, height=40, text="Top-Up", text_color="black", font=("tahoma", 16), command=lambda: self.top_up(holder))
    ...
    def top_up(self, holder: Holder):
        top_up_window = TopUpWndow(self, holder, self)
        top_up_window.mainloop()

    def update_balance(self, value: float) -> None:
        self.balance_frame.amount_label.configure(text=value)
